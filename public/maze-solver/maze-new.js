...
function solveMaze() {
  ...
  switch (selectedAlgorithm) {
    case 'astar':
      ...
      break;
    case 'dijkstra':
      path = dijkstraSearch(startNode, endNode);
      break;
    case 'bfs':
...
document.getElementById('maze-size').addEventListener('input', function() {
  mazeSize = parseInt(this.value);
  document.getElementById('size-value').textContent = `${mazeSize} x ${mazeSize}`;
});

document.getElementById('wall-density').addEventListener('input', function() {
  wallDensity = parseInt(this.value);
  document.getElementById('density-value').textContent = `${wallDensity}%`;
});
